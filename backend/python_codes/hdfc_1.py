...
def hdfc_digitization(pdf_path, pdf_password):
    ...
    #exporting the file
    #final['image_name'] = os.path.basename(pdf_path)

    final['bank_name'] = 'HDFC'
    
    final['lid'] = os.path.basename(pdf_path).split('_')[0]

    final.to_csv(r'C:\Users\hardi\OneDrive\Documents\Project Nishchay\Digitization\{}_b.csv'.format(os.path.basename(pdf_path).replace('.pdf','')), index=False)

    file_path = r'C:\Users\hardi\OneDrive\Documents\Project Nishchay\Digitization\{}_b.csv'.format(os.path.basename(pdf_path).replace('.pdf',''))
