...
def get_itrv_data(pdf_path):
    ...
    df_out['lid'] = os.path.basename(pdf_path).split('_')[0]
    df_out['customer_id'] = 0
    df_out['image_name'] = os.path.basename(pdf_path)
   
    x = pdf_path.split('_')[0]
    y = pdf_path.split('_')[1:]
    z = x + '_itr_' + '_'.join(y)
    df_out.to_csv(r'D:\digitizedfiles\{}_i.csv'.format(os.path.basename(z).replace('.pdf','')), index=False)
 
    file_path = r'D:\digitizedfiles\{}_i.csv'.format(os.path.basename(z).replace('.pdf',''))
